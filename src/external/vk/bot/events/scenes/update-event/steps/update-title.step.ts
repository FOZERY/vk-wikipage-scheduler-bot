...
> = async (context) => {
	if (context.scene.step.firstTime) {
		logStep(context, `User ${context.senderId} -> entered update-title step`, "info");
	...
	if (context.hasMessagePayload) {
		...
		if (parsedTitle.length > 255) {
			logStep(context, `User ${context.senderId} -> too long (>255) title`, "info");
	...
	logStep(context, `User ${context.senderId} -> passed update-title step`, "info");
	return await context.scene.step.go(UpdateEventSceneStepNumber.SelectFieldOrConfirm);
};
